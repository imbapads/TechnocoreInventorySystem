...
public static String[][] executeDQL(String table, String[] columns, String where){
 ...
 public static String[][] sortA(String table, String column){
        String[][] records = null;
        try{
            SQLiteOutStock.stmt = conn.createStatement();
...
public static void main(String [] args){
    ...
    int OutStockID = 1;//If you receive an error, probably sqlite detect duplicate ID value.
    String BrandID = "";
    String BrandName = "";
    String Category =  "";
    String Description = "";
    ...
    if(create("OutStock",OutStockID+","+"'"+BrandID+"'"+","+"'"+BrandName+"'"+","+"'"+Category+"'"+","+"'"+Description+"'"+","+Quantity+","+"'"+Date+"'"+","+"'"+Remarks+"'")){
    ...
    int OutStockID= 1;
     String BrandID = "";
    if(update("OutStock", "BrandID='"+BrandID+"'", OutStockID)){
    ...
    String ProductID= "";
     String BrandName = "";
    if(update3("Product", "BrandName='"+BrandName+"'", ProductID)){
        System.out.println("Updated successfully!");
    }
    SQLiteProduct.closeDB();
        ...
        if(SQLiteOutStock.openDB()){
    int OutStockID = 1;
    if(delete("OutStock", OutStockID)){
        System.out.println("Deleted successfully!");
    }
    SQLiteOutStock.closeDB();
} 
//        if(SQLiteOutStock.openDB()){
//    String[][] r = read("OutStock");
//    System.out.println("OutOfStockID:" + r[0][0]);
//    System.out.println("ProdNo:" + r[0][1]);
//    System.out.println("qty:" + r[0][2]);
//     System.out.println("Remarks:" + r[0][3]);
//    System.out.println("date:" + r[0][4]);
//
//    SQLiteOutStock.closeDB();
//}
        }
...
public static boolean createStock(String table, String values){
        ...
        try{
            SQLiteOutStock.stmt = conn.createStatement();
            query = "INSERT INTO OutStock(BrandID,BrandName,Category,Description,Quantity,Date,Remarks) VALUES(" + values + ")";
            stmt.executeUpdate(query);            
            //You can include exception handling here. (e.g. Duplicate Data, etc.)
...
public static boolean update(String table, String set, int OutStockID){
    boolean result = false;
    try{
        SQLiteOutStock.stmt = conn.createStatement();
        String query = "UPDATE "+ table +" SET " + set + " WHERE OutStockID=" + OutStockID;
        stmt.executeUpdate(query);
        //You can include exception handling here. (e.g. Duplicate Data, etc.)
        result = true;
    }
...
public static boolean delete(String table, int OutStockID){
    boolean result = false;
    try{
        SQLiteOutStock.stmt = conn.createStatement();
        String query = "DELETE FROM "+ table + " WHERE OutStockID=" + OutStockID;
        stmt.executeUpdate(query);
        result = true;
    }
    catch(Exception e){
